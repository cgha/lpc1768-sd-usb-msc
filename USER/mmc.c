...
#define CS_LOW()  GPIO_ClearValue(CFG_CSPORT, 1<<CFG_CSPIN);
#define CS_HIGH() GPIO_SetValue(CFG_CSPORT, 1<<CFG_CSPIN);

#define	FCLK_SLOW()	sspClockSlow()				/* Set slow clock (100k-400k) */
#define	FCLK_FAST()	sspClockFast()				/* Set fast clock (depends on the CSD) */
...
)
{
	...
	GPIO_SetDir( CFG_CSPORT, CFG_CSPIN, 1 ); /* CS, Output */
#ifdef STA_ENABLE_DETECT
	GPIO_SetDir( CFG_SDCARD_CDPORT, CFG_SDCARD_CDPIN, 0 ); /* Card Detect, Input */
#endif
...
void disk_timerproc (void)
{
  ...
  pv = (GPIO_ReadValue(CFG_SDCARD_CDPORT) & (1<<CFG_SDCARD_CDPIN))?1:0 ;
