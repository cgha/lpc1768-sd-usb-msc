...
#else
 uint32_t USB_SetClrFeature (uint32_t sc) {
#endif
  uint32_t n, m;
...
#else
 uint32_t USB_GetDescriptor (void) {
#endif
  uint8_t  *pD;
  uint32_t len, n;
...
#else
 uint32_t USB_SetConfiguration (void) {
#endif
...
#else
 uint32_t USB_SetInterface (void) {
#endif
  USB_COMMON_DESCRIPTOR *pD;
