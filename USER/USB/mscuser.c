 ...
 */
#ifdef CFG_SDCARD
uint8_t temp_block[MSC_BlockSize];
uint32_t temp_cache=0xFFFFFFFF;
#endif
...
uint32_t temp_cache=0xFFFFFFFF;
  ...
  // Make sure we're not reading more than 64 bytes at a time
...
#ifdef CFG_SDCARD
  // Buffer the block from the SD card if it hasn't been read yet
  blocknum = Offset/MSC_BlockSize;
  if (temp_cache != blocknum) {
    temp_cache = blocknum;
    disk_read(0, temp_block, blocknum, 1);
  }

  // Write data to EP in 64 byte chunks
  USB_WriteEP(MSC_EP_IN, &temp_block[Offset % MSC_BlockSize], n);
...
void MSC_MemoryWrite (void) {
  ...
  if ((Offset + BulkLen) > MSC_MemorySize) {
	...
	for (n = 0; n < BulkLen; n++) {
#ifdef CFG_SDCARD
		temp_block[n+Offset%MSC_BlockSize] = BulkBuf[n];
#else
...
#ifdef CFG_SDCARD
	if (((n+Offset%MSC_BlockSize) == MSC_BlockSize) || (Length == 0))
	{
		// Write block to disk
		disk_write(0,temp_block,blocknum,1);
	}
#endif  // CFG_SDCARD
...
#ifdef CFG_SDCARD
	// ToDo: Verify contents on SD card
	CSW.bStatus = CSW_CMD_PASSED;
#else
	uint32_t n;
...
void MSC_ReadCapacity (void) {
  ...
  /* Last Logical Block */
  BulkBuf[ 0] = ((sectorCount - 1) >> 24) & 0xFF;
  BulkBuf[ 1] = ((sectorCount - 1) >> 16) & 0xFF;
  BulkBuf[ 2] = ((sectorCount - 1) >>  8) & 0xFF;
  BulkBuf[ 3] = ((sectorCount - 1) >>  0) & 0xFF;
#else
  /* Last Logical Block */
...
void MSC_ReadFormatCapacity (void) {
  ...
  /* Block Count */
  BulkBuf[ 4] = (sectorCount >> 24) & 0xFF;
  BulkBuf[ 5] = (sectorCount >> 16) & 0xFF;
  BulkBuf[ 6] = (sectorCount >>  8) & 0xFF;
  BulkBuf[ 7] = (sectorCount >>  0) & 0xFF;
#else
  /* Block Count */
